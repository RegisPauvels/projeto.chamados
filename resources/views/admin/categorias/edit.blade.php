<x-app-layout>
    ...
    <div class="py-6 container mx-auto">
        <form action="{{ route('categorias.update', $category->id) }}" method="POST" class="max-w-md mx-auto bg-white p-6 rounded-lg shadow-sm">
            @csrf
            @method('PUT')
            <div class="mb-4">
            ...
            <div class="mb-4">
                ...
                <textarea name="description" rows="3"
                          class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-green-500 focus:border-transparent text-sm">{{ old('description', $category->description ?? '') }}</textarea>
            </div>
